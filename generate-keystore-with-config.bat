...
if %ERRORLEVEL% EQU 0 (
    ...
    echo # 生产环境签名配置 > android/gradle.properties.temp
    echo MYAPP_RELEASE_STORE_FILE=release.keystore >> android/gradle.properties.temp
    echo MYAPP_RELEASE_KEY_ALIAS=yuzutang_release_key >> android/gradle.properties.temp
    echo MYAPP_RELEASE_STORE_PASSWORD=yuzutangdeapp.2024 >> android/gradle.properties.temp
    echo MYAPP_RELEASE_KEY_PASSWORD=yuzutangdeapp.2024 >> android/gradle.properties.temp
    
    type android/gradle.properties >> android/gradle.properties.temp
    move /y android/gradle.properties.temp android/gradle.properties
    ...
    echo.
    echo 🎉 证书生成和配置完成！
    echo 💡 下一步：运行 build-production.bat 进行打包
) else (
    echo ❌ 证书生成失败！
